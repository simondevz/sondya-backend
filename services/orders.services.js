/* eslint-disable no-undef */
import transporter from "../config/email.js";
import dotenv from "dotenv";
dotenv.config();

// user email for orders made
export const sendOrdersMadeEmail = (
  to,
  {
    username,
    order_id,
    order_status,
    product,
    service,
    total_cost,
    date_ordered,
  }
) => {
  // email template
  const serviceTemplate = `<li><strong>Service name:</strong> ${service?.name}</li><li><strong>Service Vendor:</strong> ${service?.seller_name}</li>`;
  const productTemplate = `<li>
    <strong>Product:</strong>
    <ul>${product?.map((item) => {
      return `
        <li><strong>name:</strong> ${item?.name}</li>
        <li><strong>Quantity:</strong> ${item?.name}</li>
        <li><strong>Vendor:</strong> ${item?.seller_name}</li>
        `;
    })}</ul>
  </li>`;
  const ordersMadeEmailTemplate = `<!DOCTYPE html>
  <html>
    <head>
      <title>Order Notification</title>
      <style>
        body {
          font-family: Arial, sans-serif;
          background-color: #f4f4f4;
          text-align: center;
        }
        .container {
          max-width: 600px;
          margin: 0 auto;
          padding: 20px;
          background-color: #fff;
          border: 1px solid #ddd;
          border-radius: 5px;
        }
        h1 {
          color: #333;
        }
        p {
          color: #777;
        }
        .logo {
          max-width: 150px;
          margin: 0 auto;
          display: block;
        }
  
        .main_div {
          text-align: start;
        }
      </style>
    </head>
    <body>
      <div class="container">
        <img
          class="logo"
          src="data:image/png;base64,"
          alt="Sondya Logo"
        />
        <h2>Order Confirmation Notification</h2>
        <div class="main_div">
          <p>Dear ${username},</p>
  
          <p>
            We wanted to let you know that your recent order with Sondya has been
            successfully received and is now being processed.
          </p>
  
          <p><strong>Order Details:</strong></p>
          <ul>
            ${order_id ? `<li><strong>Order ID:</strong> ${order_id}</li>` : ""}
            <li><strong>Order Date:</strong> ${date_ordered}</li>
            <li><strong>Order Status:</strong> ${order_status}</li>
            ${service ? serviceTemplate : productTemplate}
            <li><strong>Total Cost:</strong> $${total_cost}</li>
          </ul>
  
          <p>
            If you have any questions or need assistance, feel free to contact our
            support team at
            <a href="mailto:support@sondya.com">support@sondya.com</a>. We're here
            to ensure a smooth and hassle-free experience for you.
          </p>
        </div>
  
        <p>Thank you for choosing Sondya.</p>
        <p>
          Best regards,<br />
          Sondya Support Team
        </p>
      </div>
    </body>
  </html>
  `;

  // mail options
  const mailOptions = {
    from: `"Support Sondya" <${process.env.EMAIL_USERNAME}>`, // sender address
    to: to, // list of receivers
    subject: "Order Confirmation Notification", // Subject line
    html: ordersMadeEmailTemplate, // html body
  };

  // send mail with defined transport object
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      console.log(error);
      return error;
    } else {
      console.log(info.response);
      return info.response;
    }
  });
};
